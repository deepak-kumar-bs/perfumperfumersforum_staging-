...
//            $params['location'] = $_GET['location'];
        //location work...........................................
        if (isset($params['viewType']) && !empty($params['viewType'])) {
            if (isset($params['restapilocation']) & !empty($params['restapilocation'])) {
                $params['location'] = $params['restapilocation'];
                $params['locationmiles'] = Engine_Api::_()->getApi('settings', 'core')->getSetting('seaocore.locationdefaultmiles', 0);
            } 
            $params['restapimap_view'] = 1;
        } else {
            if (isset($params['restapilocation']) & !empty($params['restapilocation'])) {
                $params['location'] = $params['restapilocation'];
                $params['locationmiles'] = Engine_Api::_()->getApi('settings', 'core')->getSetting('seaocore.locationdefaultmiles', 0);
        ...
        if ($enabledModules && !in_array('sitereview', $enabledModules) && empty($params['viewType'])) {
            unset($params['location']);
            unset($params['locationmiles']);
        }

        if (isset($_GET['location']) && !empty($_GET['location'])) {
            $params['locationmiles'] = isset($_GET['locationmiles']) ? $_GET['locationmiles']:Engine_Api::_()->getApi('settings', 'core')->getSetting('seaocore.locationdefaultmiles', 0);
            $params['location'] = $_GET['location'];
        }


        if ((isset($params['user_id']) && $params['user_id']) || (isset($params['category_id']) && !empty($params['category_id'])) || $params['listingtype_id'] == '18') {
        ...
        if (isset($params['review']))
            $params['has_review'] = $params['review'];
        //............................................................................
        ...
        if ($viewer->getIdentity() && !empty($row) && !empty($row->display) && isset($params['show']) && $params['show'] == 3 && !isset($_GET['show'])) {
            $params['show'] = 3;
        }
        ...
        try {
            ...
            } elseif (isset($params['listing_filter']) && $params['listing_filter'] == 'favourite') {
            ...
            foreach ($paginator as $listing) {
                ...
                if (!empty($listing->category_id)) {
                    ...
                    $listingArray['categoryTitle'] = Engine_Api::_()->getItem('sitereview_category', $listing->category_id)->getTitle();
                }
        ...
        try {
            ...
            foreach ($paginator as $listing) {
                ...
                if (method_exists('Siteapi_Api_Core', 'isSitevideoPluginEnabled')) {
                    $subject_type = $listing->getType();
                    $subject_id = $listing->getIdentity();
                    ...
                    if (!empty($advVideoEnableArray) && is_array($advVideoEnableArray)) {
                        $listingArray = array_merge($listingArray, $advVideoEnableArray);
                    } else {
    ...
    public function createAction() {
        ...
        if (empty($pakage_id)) {
            $pakage_id = $this->_sitereviewPackageEnabled($this->_listingTypeId);
        }
...
//        $bodyParams = array();
        ...
        if ($this->getRequest()->isGet()) {
            $response = Engine_Api::_()->getApi('Siteapi_Core', 'Sitereview')->getListingCreateForm($this->_listingTypeId, null, null, $package_id);
            ...
            try {
                ...
                if (isset($data['end_date_enable']))
                    $values['end_date_enable'] = $values['end_date_enable'] - 1;
                ...
                if (isset($values['auth_event']) && !empty($values['auth_event'])) {
                    ...
                    foreach ($roles as $i => $role) {
                        $auth->setAllowed($sitereview, $role, "event_listtype_$this->_listingTypeId", ($i <= $eventMax));
                    }
                }
                ...
                foreach ($roles as $i => $role) {
                    $auth->setAllowed($sitereview, $role, "topic_listtype_$this->_listingTypeId", ($i <= $topicMax));
                    $auth->setAllowed($sitereview, $role, "photo_listtype_$this->_listingTypeId", ($i <= $photoMax));
                    $auth->setAllowed($sitereview, $role, "video_listtype_$this->_listingTypeId", ($i <= $videoMax));
                }
                $db->commit();
            } catch (Exception $e) {
                $db->rollBack();
...
//        $sitereviewListingTypeOrder += $sitereviewFeaturedOrder + $sitereviewSponsoredOrder;
        ...
        $previous_location = $sitereviewObj->location;
        try {
        ...
        if (isset($sitereview['end_date_enable']))
            $sitereview['end_date_enable'] = $sitereview['end_date_enable'] + 1;
        ...
        if ($this->getRequest()->isGet()) {
            ...
            $db->beginTransaction();
            try {
                if (isset($values['end_date_enable']))
                    $values['end_date_enable'] = $values['end_date_enable'] - 1;
                ...
                $db->commit();
                //edit location.........
                $this->EditLocation($previous_location, $sitereviewObj);
                $db->beginTransaction();
            ...
            if (isset($sitereviewObj) && !empty($sitereviewObj)) {
                ...
                if (!empty($this->_listingType->overview)) {
                    ...
                    if (!empty($overview)) {
                        ...
                        //Dom Object to get src attributes
                         $dom = new Zend_Dom_Query($overview);
                        // Find all images
                          $imageQuery = $dom->query('img');
                          $allSrc = array();
                        foreach ($imageQuery as $image) {
                            $allSrc[] = $image->getAttribute('src');

                        }
                        if (!empty($allSrc)) {
                            foreach ($allSrc as $element) {
                                $src = $element;
                                if (!(stristr($src, 'http://') || stristr($src, 'https://'))) {
                                    $sitereview['overview'] = str_replace($src, $this->getHost . '/' . $src, $sitereview['overview']);
                                }
...
//                    }
                ...
                $params = array();
                $params['listing_id'] = $sitereviewObj->getIdentity();
                $params['viewer_id'] = $viewer_id;
                $applyStatus = Engine_Api::_()->getDbtable('jobs', 'sitereview')->getApplyStatus($params);
                $apply = Engine_Api::_()->getDbTable('listingtypes', 'sitereview')->getListingTypeColumn($this->_listingTypeId, 'allow_apply');
                if(!empty($apply) && ($viewer->getIdentity() != $sitereviewObj->owner_id)){
                    if(empty($applyStatus)) 
                        $sitereview['isApplyJob'] = 0;
                    else
                        $sitereview['isApplyJob'] = 1;
                }
    ...
    private function _gutterMenus($subject, $listingTypeId) {
        ...
        if (!empty($viewer_id)) {
            $tempMenu[] = array(
             ...
             $viewType = $this->_getViewTypeLabel($this->_listingTypeId, 1);
            if($viewType != 1)
            {
...
//            SHOW MESSAGE OWNER LINK TO USER IF MESSAGING IS ENABLED FOR THIS LEVEL     
        ...
        if (Engine_Api::_()->sitereview()->hasPackageEnable() && !empty($viewer_id) && !empty($owner_id) && $owner_id == $viewer_id && (_CLIENT_TYPE && ((_CLIENT_TYPE == 'android' && _ANDROID_VERSION >= '1.8.7') || _CLIENT_TYPE == 'ios' && _IOS_VERSION >= '1.8.0'))) {
            $tempMenu[] = array(
                'label' => $this->translate('Upgrade Package'),
                'name' => 'upgrade_package',
                'url' => 'listings/upgrade-package',
        ...
        if (_CLIENT_TYPE && ((_CLIENT_TYPE == 'android' && _ANDROID_VERSION >= '1.8.2') || _CLIENT_TYPE == 'ios' && _IOS_VERSION >= '1.6.6')) {
            if($viewer_id != $owner_id){
                ...
                $params['viewer_id'] = $viewer_id;
                $applyStatus = Engine_Api::_()->getDbtable('jobs', 'sitereview')->getApplyStatus($params);
                if (!empty($apply) && empty($applyStatus)) {
...
//        }
    ...
    private function _profileTAbsContainer($subject) {
        ...
        if ($viewType == 2) {
            return $this->_classifiedMenus($subject);
        }
        ...
        if (method_exists('Siteapi_Api_Core', 'isSitevideoPluginEnabled')) {
            ...
            $subject_id = $subject->getIdentity();
            $advVideoEnableArray = Engine_Api::_()->getApi('Core', 'siteapi')->isSitevideoPluginEnabled($subject_type, $subject_id);
            if (isset($advVideoEnableArray['sitevideoPluginEnabled']) && !empty($advVideoEnableArray['sitevideoPluginEnabled']) && _CLIENT_TYPE && ((_CLIENT_TYPE == 'android' && _ANDROID_VERSION >= '3.2.3') || _CLIENT_TYPE == 'ios' && _IOS_VERSION > '2.1.6')) {
        ...
        $location = $this->getMLTLocation($subject);
        if (!empty($location) && ( _ANDROID_VERSION >= '3.0' || _IOS_VERSION > '2.4'))
            $response[] = array(
                'name' => 'map',
...
//                $listing_title_with_link = '<a href =' . $http . $_SERVER['HTTP_HOST'] . Zend_Controller_Front::getInstance()->getRouter()->assemble(array('listing_id' => $listing_id, 'slug' => $sitereview->getSlug())) . ">$listing_title</a>";
    ...
    public function categoriesAction() {
        ...
        if ($showCategories) {
            ...
            if ($showAllCategories) {
                $category_info = $tableCategory->getCategories(null, 0, $this->_listingTypeId, 0, 1, 0, $orderBy, 1, array('category_id', 'category_name', 'cat_order', 'listingtype_id', 'file_id', 'photo_id'));
        ...
        if ($showListings && isset($category_id) && !empty($category_id)) {
            ...
            //GET PAGE RESULTS
            $params['paginator'] = 1;
            $params['limit'] = $this->getRequestParam("limit", 20);
            $params['page'] = $this->getRequestParam("page", 1);
    ...
    private function _freePackageEnabled($listingType_id) {
        $viewer = Engine_Api::_()->user()->getViewer();
        $viewer_id = $viewer->getIdentity();
        $package_id = 0;
        //GET USER LEVEL ID
    ...
    private function _classifiedMenus($subject) {
        $response = array();
        $response[] = array(
                'name' => 'overview',
                'label' => $this->translate('Overview'),
    ...
    private function _getViewTypeLabel($listingtype_id, $viewType = 1) {
         ...
         */
        if (!empty($row))
            return (!isset($row->profileView_id)) ? 0 : $row->profileView_id;
        else
            return 0;
    }

    public function getMLTLocation($subject) {
        ...
        $select = $select
                ...
                ->join($locationTableName, "$sitereviewTableName.listing_id = $locationTableName.listing_id");
        $select->where("`{$locationTableName}`.formatted_address LIKE ? or `{$locationTableName}`.location LIKE ? or `{$locationTableName}`.city LIKE ? or `{$locationTableName}`.state LIKE ?", "%" . $subject->location . "%")
                ->where("$sitereviewTableName.listing_id =?", $subject->getIdentity());
        $Result = $sitereviewTable->fetchRow($select);
        return $Result;
    }

    public function EditLocation($previous_location, $sitereviewObj) {
        $showLocation = 1;

        if (Engine_Api::_()->sitereview()->hasPackageEnable()) {
            if (Engine_Api::_()->sitereviewpaidlisting()->allowPackageContent($sitereviewObj->package_id, "map"))
                $showLocation = 1;
            else
                $showLocation = 0;
        }
        if ($this->_listingType->location && !empty($showLocation)) {
            $location = $_REQUEST['location'];
            $locationTable = Engine_Api::_()->getDbtable('locations', 'sitereview');
            if (!empty($location) && $location != $previous_location) {
                $sitereviewObj->setLocation();
            } elseif (empty($location)) {
                $locationTable->delete(array('listing_id = ?' => $sitereviewObj->getIdentity()));
            }
        }
    }
