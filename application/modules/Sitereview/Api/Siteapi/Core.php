...
class Sitereview_Api_Siteapi_Core extends Core_Api_Abstract {
     ...
     * @return array
     */
    public function getListingCreateForm($listingtype_id, $item = array(), $profileType = null, $pakage_Id = null) {
        if (Engine_Api::_()->sitereview()->hasPackageEnable()) {
            if ($item) {
                $pakage_id = $item->package_id;
            } else {
                $pakage_id = Zend_Controller_Front::getInstance()->getRequest()->getParam('package_id', null);
                if (empty($pakage_id)) {
                    $pakage_id = $pakage_Id;
                }
            }
...
//            'attribs' => array('style' => 'max-height:150px; '),
        ...
        if ($listingTypeObj->expiry == 1) {
            $createForm[] = array(
                ...
                'multiOptions' => array("1" => "No end date.", "2" => "End $listing_singular_lc on a specific date. (Please select date by clicking on the calendar icon below.)"),
...
//        }
    ...
    public function getApplyNowForm($listingtype_id) {
        ...
        $applyNowForm[] = array(
            'type' => 'Text',
            'name' => 'contact',
            'label' => $this->translate('Contact Number'),
            'hasValidator' => true
        );
