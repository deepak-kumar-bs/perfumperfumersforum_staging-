...
class Sitetagcheckin_Widget_MapSitetagcheckinController extends Engine_Content_Widget_Abstract {
  ...
  public function indexAction() {
    ...
    $totalMapLocation = false ;$locations = false ; $this->view->updates = false ;
    $this->view->totalUpdates = false ; $this->view->checkins = false ;
    $this->view->totalCheckins = false ; $this->view->taggedPhotos = false ;
    $this->view->totalTaggedPhotos = false ;

    // GET ACTION IDS :
    $action_ids = $addlocationsTable->getActivity($subject, $viewer);

    if(!empty($action_ids)){
        //GET FILTER BASE COUNT
        $totalMapLocation = $addlocationsTable->getFilterBasedCount($action_ids,$subject, 'all');
        //GET ALL LOCATIONS 
        $locations = $addlocationsTable->getAllLocations($action_ids,$subject, 'all');

        //GET ALL CHECKIN UPDATES
        $this->view->updates = $addlocationsTable->getAllLocations($action_ids,$subject, 'checkin');

        //COUNT TOTAL UPDATES
        $this->view->totalUpdates = $addlocationsTable->getFilterBasedCount($action_ids,$subject, 'checkin');

        //GET ALL CHECKINS
        $this->view->checkins = $addlocationsTable->getAllLocations($action_ids,$subject, 'checkin', 'parent_id', 1);

        //GET TOTAL CHECKINS
        $this->view->totalCheckins = $addlocationsTable->getFilterBasedCount($action_ids,$subject, 'checkin', 'parent_id');

        //GET TAGGED PHOTOS
        $this->view->taggedPhotos = $addlocationsTable->getAllLocations($action_ids,$subject, 'tagging');

        //GET TAGGED PHOTOS COUNT
        $this->view->totalTaggedPhotos = $addlocationsTable->getFilterBasedCount($action_ids,$subject, 'tagging');
    }
