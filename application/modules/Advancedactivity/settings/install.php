...
class Advancedactivity_Installer extends Engine_Package_Installer_Module {
    ...
    function onPreinstall() {
        ...
        $SocialEngineAddOns_version = '4.10.3p7';
    ...
    function onInstall() {
        ...
        if ($table_engine4_album_albums_exist) {
            ...
            if (!empty($album_type)) {
                ...
                if (empty($this->checkVersion($getModVersion->version, '4.8.13'))) {
        ...
        $select = new Zend_Db_Select($db);
        $check_sitepage = $select
                ->from('engine4_core_modules', 'version')
                ->where('name = ?', 'sitepage')
                ->where('enabled = ?', 1)
                ->query()
                ->fetchcolumn();
        if (!empty($this->checkVersion($check_sitepage, '4.2.3')) && !empty($sitepage_is_active)) {
...
(\'aaf_tagged\', \'advancedactivity\', \'{item:$subject} tagged your {var:$item_type} in a {item:$object:$label}.\', 0, \'\', 1);');
        ...
        $select = new Zend_Db_Select($db);
        $check_advfeedversion = $select
                ->from('engine4_core_modules', 'version')
                ->where('name = ?', 'advancedactivity')
                ->query()
                ->fetchColumn();
        if (empty($this->checkVersion($check_advfeedversion, '4.2.6'))) {
            $select = new Zend_Db_Select($db);
        ...
        if (!empty($is_sitemobile_object)) {
            ...
            if (($this->checkVersion($check_advfeedversion, '4.6.0')) != -1) {
                $select = new Zend_Db_Select($db);
                $select
  ...
  `mute_story` INT(11) NOT NULL DEFAULT '0',
  PRIMARY KEY (`story_id`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;");
        } else {
            $story_description = $db->query("SHOW COLUMNS FROM engine4_advancedactivity_stories LIKE 'description'")->fetch();
            if (!empty($story_description)) {
                ...
                if ($getModVersion->version < '4.10.3p9') {
                    $db->query("ALTER TABLE `engine4_advancedactivity_stories` CHANGE `description` `description` blob;");
                }
            }

            $story_mute_story = $db->query("SHOW COLUMNS FROM engine4_advancedactivity_stories LIKE 'mute_story'")->fetch();
            if (empty($story_mute_story)) {
                $db->query("ALTER TABLE `engine4_advancedactivity_stories` ADD `mute_story` INT(11) NOT NULL DEFAULT '0';");
            }
        }
  ...
  PRIMARY KEY (`viewer_id`)
        ...
        $storymute_table_exist = $db->query("SHOW TABLES LIKE 'engine4_advancedactivity_mutestories'")->fetch();
        if (empty($storymute_table_exist)) {
            $db->query("CREATE TABLE `engine4_advancedactivity_mutestories` (
  `mutestory_id` int(11) NOT NULL auto_increment,
  `story_id` int(11) NOT NULL,
  `user_id` int(11) NOT NULL,
  `create_date` datetime NOT NULL,
  PRIMARY KEY (`mutestory_id`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;");
        }

        //For mute user
        $storyuser_table_exist = $db->query("SHOW TABLES LIKE 'engine4_advancedactivity_muteusers'")->fetch();
        if (empty($storyuser_table_exist)) {
            $db->query("CREATE TABLE `engine4_advancedactivity_muteusers` (
                        `muteuser_id` int(11) NOT NULL AUTO_INCREMENT,
                         `user_id` int(11) UNSIGNED NOT NULL,
                         `muted_user_id` int(11) UNSIGNED NOT NULL,
                         PRIMARY KEY (`muteuser_id`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;");
        }
    ...
    public function onDisable() {
        ...
        try {
            ...
            // Disable Live streaming plugin
            $db->query('UPDATE `engine4_core_modules` SET `enabled` = "0" WHERE `engine4_core_modules`.`name` = "sitelivestreaming" LIMIT 1');
            $db->query('UPDATE `engine4_core_modules` SET `enabled` = "0" WHERE `engine4_core_modules`.`name` = "livestreamingvideo" LIMIT 1');
        } catch (Exception $e) {
            die(" Exception " . $e);
        }
    }
