...
class Advancedactivity_FeedController extends Siteapi_Controller_Action_Standard {
    ...
    public function init() {
        ...
        if (0 !== ($subject_id = (int) $this->getRequestParam('subject_id')) && null !== ($subject = Engine_Api::_()->getItem($subject_type, $subject_id))) {
        ...
        $action_id = (int) $this->getRequestParam('action_id', 0);
        if (0 !== ($action_id) && (!isset($subject) || empty($subject))) {
            try {
                $actionTable = Engine_Api::_()->getDbtable('actions', 'advancedactivity');
                $action = $actionTable->getActionById($action_id);
                if(empty($action)){
                    $this->respondWithError('unauthorized',"Feed has been deleted");
                }
    ...
    public function indexAction() {
        ...
        $onlyMyDeviceVideo = $this->_getparam('onlyMyDeviceVideo', 0);
...
//    if ( empty($feedOnly) && Engine_Api::_()->core()->hasSubject() )
        ...
        if (Engine_Api::_()->core()->hasSubject()) {
            ...
            if (empty($action_id) && !empty($isAllowedToView))
                $this->respondWithError('unauthorized');
        }
        // Activity feed not be more then from 50
        $length = ($length > 50) ? 50 : $length;
        // Set the Content List
        if (!empty($feed_filter) && empty($feedCountOnly)  && $this->getRequestParam('filter_type') && $viewer_id) {
...
//    if ( !empty($feed_filter) && empty($feedCountOnly) && empty($feedOnly) && empty($subject) && !$isForCategoryPage ) {
        if (!empty($feed_filter) && empty($feedCountOnly) && empty($feedOnly)) {
            if (!empty($viewer_id)) {
            ...
            foreach ($contentTabs as $value) {
                ...
                if ((_CLIENT_TYPE && (_CLIENT_TYPE == 'ios') && _IOS_VERSION && _IOS_VERSION < '2.3.1') || (_CLIENT_TYPE == 'both')) {
                ...
                if ($actions && count($actions) > 0) {
                    foreach ($actions as $action) {
                        ...
                        if (!$action->getObject() || !$action->getObject()->getIdentity())
                            continue;
                        $feedOwner = $action->getSubject();
                        $feedOwnerId = $action->getSubject()->getIdentity();
                        if($feedOwner->getType() == 'user'){
                            $feedOwnerObj = Engine_Api::_()->getItem('user', $feedOwnerId);
                        }
                        if($viewer->isBlocked($feedOwnerObj))
                            continue;
                        ...
                        //Video suggestion (Only for my Computer)
                        $videoType=0;
                        if (!empty($onlyMyDeviceVideo) && $defaultcontentTab == 'video') {
                            foreach ($action->getAttachments() as $attachment) {
                                $videoType = Engine_Api::_()->getApi('Siteapi_Feed', 'advancedactivity')->videoType($attachment->item->type);
                                
                            }
                            
                            try{
                            $canViewVideo = Engine_Api::_()->authorization()->isAllowed($attachment->item, $viewer, 'view');
                            } catch (Exception $ex) {
                                $canViewVideo = 0;
                            }
                          
                            if($videoType !=3 || !$canViewVideo)
                            continue;
                        }
...
//            $this->respondWithSuccess();
        ...
        $getBodyResponse['activityCount'] = ($getFeeds) ? count($getFeeds): 0;
        ...
        if (_ANDROID_VERSION >= '2.3' || _ANDROID_VERSION > '1.5') {
        ...
        try {
            ...
            if (Engine_Api::_()->getApi('Siteapi_Feed', 'advancedactivity')->isSitestickerPluginLive()) {
                ...
                $composerList = @implode(",", Engine_Api::_()->getApi('settings', 'core')->getSetting('advancedactivity.composer.menuoptions', array('photoXXXalbum', 'linkXXXcore', 'musicXXXmusic', 'checkinXXXsitetagcheckin', 'videoXXXvideo', "sitevideoXXXsitevideo")));
                if (strstr($composerList, $modName)) {
        ...
        if(empty($getPostMenus)){
            unset($getBodyResponse['feed_post_menu']);
        }
        
        $getBodyResponse['notifyItemAt'] = isset($_REQUEST['notifyItemAt']) ? $_REQUEST['notifyItemAt'] : 0;
        ...
        // Success
        if (!empty($viewer_id))
            $this->respondWithSuccess($getBodyResponse, true);
        else
    ...
    public function postAction() {
        ...
        if (isset($postData['min_age']) && isset($postData['max_age']) && $postData['min_age'] > $postData['max_age']) {
            $this->respondWithError('unauthorized', "Min age should be less than Max age.");
        }
        ...
        try {
            ...
            if (isset($post_attach) && ($post_attach == 1)) {
                ...
                //to attach link
                if ($type == 'link') {
                    if(!empty($_REQUEST['linkInfo']))
                     $link = Zend_Json::decode($_REQUEST['linkInfo']);
                    else
                        {
                    ...
                    if (isset($link) && !empty($link)) {
                        ...
                        try {
                            ...
                            if(empty($link['thumb']) && $link['imageCount']>0){
                               
                                $link['thumb'] = $link['images'][0];
                            }
                ...
                } else if ($type == 'video' && isset($_REQUEST['video_id']) && !empty($_REQUEST['video_id'])) {
                    $attachmentData['video_id'] = $_REQUEST['video_id'];
                    $video = Engine_Api::_()->getItem('video', $_REQUEST['video_id']);
                ...
                } else if ($type == 'music' && isset($_REQUEST['song_id']) && !empty($_REQUEST['song_id'])) {
                    $plugin = Engine_Api::_()->loadClass('Music_Plugin_Composer');
                    $method = 'onAttachMusic';
                    $attachmentData['song_id'] = $_REQUEST['song_id'];
                    $song = Engine_Api::_()->getItem('music_playlist_song', $_REQUEST['song_id']);
                    if (isset($song) && !empty($song)) {
                        $attachmentData['title'] = $song->title;
                        $attachment = $plugin->$method($attachmentData);
                    }
                }
                elseif($type == 'gifphoto')
                    {
                        $plugin = Engine_Api::_()->loadClass('Sitegifplayer_Plugin_Composer_GifPhoto');
                        $method = 'onAttachGifPhoto';
                        $attachmentData['gif_guid'] = $postData['gif_guid'];
                        $attachmentData['gif_code'] = $postData['code'];
...
//             }
                    ...
                    if ($action && $attachment) {
                        ...
                        $count = 0;
                        if ((isset($attachmentData['type']) && $attachmentData['type'] == 'photo') && $photoCount > 1) {
            ...
            if ($action && isset($postData['toValues']) && !empty($postData['toValues'])) {
                $actionTag = new Engine_ProxyObject($action, Engine_Api::_()->getDbtable('tags', 'advancedactivity'));
                $users = array_values(array_unique(explode(",", $postData['toValues'])));
            ...
            //Target User
            if ((isset($postData['min_age']) && isset($postData['max_age'])) && (!(empty($postData['min_age']) && empty($postData['max_age']) && empty($postData['who'])) || $postData['min_age'] < $postData['max_age'])) {
            ...
            if (((_IOS_VERSION && _IOS_VERSION <= '2.5.17') && (_ANDROID_VERSION && _ANDROID_VERSION < '3.0')))
        ...
        $this->_forward('index', 'feed', 'advancedactivity', array(
            'action_id' => $action->getIdentity(),
            'object_info' => 1,
            'getAttachedImageDimention' => 0,
            'feed_filter' => 0,
        ));
    }
...
// Throw error for logged-out user.
        if (!$this->_helper->requireUser()->isValid())
            $this->respondWithError('unauthorized');
        if (null == ($type = $this->getRequestParam('type', null)) || null == ($id = $this->getRequestParam('id', null))) {
...
// Likes
        ...
        $canComment = ($action->getTypeInfo()->commentable && $action->commentable && $viewer->getIdentity() && Engine_Api::_()->authorization()->isAllowed($action->getCommentObject(), null, 'comment')) ? 1 : 0;
        $canDelete = $action->authorization()->isAllowed($viewer, 'edit');
// If has a page, display oldest to newest
        if (null !== $page) {
            $commentSelecCount = $commentSelect = $action->comments()->getCommentSelect();
            if (!empty($comment_id)) {
                $commentSelect->where("comment_id=?", $comment_id);
            }
            $commentSelecCount->order('comment_id ' . $this->getRequestParam('order', 'ASC'));
            $commentsCount = Zend_Paginator::factory($commentSelecCount);
            $getTotalCommentCount = $commentsCount->getTotalItemCount();
            //Nested comment Work
            $enabledAdvComment = Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('nestedcomment');
            if (!empty($enabledAdvComment) && _ANDROID_VERSION > '1.6.1.1') {
            ...
            $getTotalItemCount = $comments->getTotalItemCount();
        } else {
...
// Iterate over the comments backwards (or forwards!)
            ...
            for (; $i != $e; $i += $d) {
                ...
                if (Engine_Api::_()->getApi('Siteapi_Feed', 'advancedactivity')->isSitestickerPluginLive()) {
                    if (isset($comment->attachment_type) && !empty($comment->attachment_type) && isset($comment->attachment_id) && !empty($comment->attachment_id) && ($comment->attachment_type == 'sitereaction_sticker' || $comment->attachment_type == 'album_photo' || $comment->attachment_type == 'sitegifplayer_photo')) {
                        $attachment = Engine_Api::_()->getItem($comment->attachment_type, $comment->attachment_id);
                        $params = Engine_Api::_()->getApi('Siteapi_Feed', 'advancedactivity')->getPhotoUrl($attachment, "thumb.profile");
                        $getHost = $getParentHost = Engine_Api::_()->getApi('Core', 'siteapi')->getHost();
                        $baseParentUrl = Zend_Controller_Front::getInstance()->getBaseUrl();
                        ...
                        $imageUrl = $params['url'];
                        $imageUrl = (!strstr($imageUrl, "application/modules")) ? $host . $imageUrl : $imageUrl;
            ...
            //Gutter Menu Work...................
           $commentInfo['gutterMenu'] = Engine_Api::_()->getApi('Siteapi_Core', 'advancedactivity')->gutterMenus($action,$canDelete,$poster,$comment);
           ...
           if (!empty($canDelete) || $poster->isSelf($viewer)) {
                ...
                if (!empty($enabledAdvComment) && _ANDROID_VERSION > '1.6.1.1') {
                    if (!empty($canComment)) {
                        ...
                        $replyInfo["label"] = $this->translate("Reply");
                        $replyInfo["url"] = "advancedcomments/reply";
                        $replyInfo["urlParams"] = array(
                            "action_id" => $action_id,
                            "subject_type" => $action->getType(),
                            "subject_id" => $action->getIdentity(),
                            "comment_id" => $comment->getIdentity()
                        );
                        $commentInfo["reply"] = $replyInfo;
                        $commentInfo["isReply"] = 1;
                    } else {
                        $commentInfo["reply"] = null;
                    }
                    $repyComment = $this->_getReplyCommentInfo($action, $comment->comment_id, $param);
                    if (count($repyComment) > 0) {
                        $commentInfo['reply_on_comment'] = $repyComment['allComments'];
                        $commentInfo['reply_count'] = $repyComment['totalReplyCount'];
                    }
                }
...
// FOLLOWING ARE THE GENRAL INFORMATION OF THE PLUGIN, WHICH WILL RETURN IN EVERY CALLING.
        ...
        $bodyParams['getTotalItemCount'] = $getTotalItemCount;
...
//    }
    ...
    private function _getUrlInfo($uri, Zend_Http_Response $response) {
        ...
        if ((_IOS_VERSION && _IOS_VERSION >= '2.2') || (_ANDROID_VERSION && _ANDROID_VERSION >= '2.3.6')) {
            $key = Engine_Api::_()->getApi('settings', 'core')->getSetting('sitevideo.youtube.apikey');
            if (!empty($key)) {
                if (preg_match('/(www\.|)youtube\.com\/watch/i', $uri)) {
                    $values['url'] = $uri;
                    $values['type'] = 1;
                    $video = Engine_Api::_()->getApi('Siteapi_Feed', 'advancedactivity')->postVideo($values);
                    $bodyParams = $video->toArray();
                ...
                if (preg_match('/(www\.|)youtu\.be/i', $uri)) {
                    $values['url'] = $uri;
                    $values['type'] = 1;
                    $video = Engine_Api::_()->getApi('Siteapi_Feed', 'advancedactivity')->postVideo($values);
                    $bodyParams = $video->toArray();
            ...
            if (preg_match('/https:\/\/(www\.)?vimeo\.com\/(\d+)/', $uri)) {
                $values['url'] = $uri;
                $values['type'] = 2;
                $video = Engine_Api::_()->getApi('Siteapi_Feed', 'advancedactivity')->postVideo($values);
                $bodyParams = $video->toArray();
...
// Make an array for feed post menus.
        $activityPost = array();
        $showPrivacyDropdown = Engine_Api::_()->getApi('settings', 'core')->getSetting('advancedactivity.composer.options', array("withtags", "emotions", "userprivacy"));

        $activityPost = array();
        $activityPost['status'] = 1;
        if (in_array("withtags", $showPrivacyDropdown)) {
            $activityPost['withtags'] = 1;
        } else {
            $activityPost['withtags'] = 0;
        }

        if (in_array("emotions", $showPrivacyDropdown)) {
            $activityPost['emotions'] = 1;
        } else {
            $activityPost['emotions'] = 0;
        }

        $livestreamModuleVersion = Engine_Api::_()->getDbtable('modules', 'core')->getModule('livestreamingvideo')->version;

        if (Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('livestreamingvideo') && Engine_Api::_()->siteapi()->checkVersion($livestreamModuleVersion, '5.5.0'))
        {
           $enabledBoradCast = Engine_Api::_()->getApi('Siteapi_Core', 'livestreamingvideo')->hasPackageEnable();
           $activityPost['livestreamingvideo'] = !empty($enabledBoradCast)? 1 : 0;
        }

        $composerList = @implode(",", Engine_Api::_()->getApi('settings', 'core')->getSetting('advancedactivity.composer.menuoptions', array('photoXXXalbum', 'linkXXXcore', 'musicXXXmusic', 'checkinXXXsitetagcheckin', 'videoXXXvideo', "sitevideoXXXsitevideo")));
        $moduleEnabledByUs = array('album' => 'photo', 'sitetagcheckin' => 'checkin', 'video' => 'video', 'music' => 'music', 'core' => 'link', 'sitereaction' => 'stickersEnabled');
        ...
        if (!empty($activityPost['checkin'])) {
            $activityPost['checkin'] = (Engine_Api::_()->getApi('settings', 'core')->getSetting('sitetagcheckin.status.update', 1) && !empty($activityPost['checkin'])) ? 1 : 0;
        }
        ...
        $activityPost['withtags'] = strstr($composerList, 'feed-tags') ? 1 : 0 ;

        // Gif display setting work in status box
        ...
        {
            $activityPost['gif'] = Engine_Api::_()->authorization()->isAllowed('sitegifplayer', $viewer, 'allow_gif_feed');
        }
        ...
        $activityPost['video'] = (!empty($ffmpeg_path)) && $activityPost['video'] ? 1 : 0;

        //Post feed setting work.............
        $userprivacy = Engine_Api::_()->getApi('settings', 'core')->getSetting(
        'activity.view.privacy',
        array('everyone', 'registered', 'network', 'member', 'owner')
      );
        foreach($userprivacy as $key => $privacy){
             $activityPost['userprivacy'][$privacy] = $this->translate(ucfirst($privacy));
             $activityPost['userprivacy']['onlyme'] = $this->translate('Only me');
        }
        ...
        if (_ANDROID_VERSION >= '2.3' || _IOS_VERSION >= '2.3.1') {
            ...
            $multiple_network_list = $this->_multipleNetworkList($privacyNetwork, $subject);
            if(!empty($multiple_network_list))  {
                $activityPost['multiple_networklist'] = $multiple_network_list;
...
// Get page id and object
    ...
    private function _getReplyCommentInfo($action, $comment_id, $param = array()) {
        $page = $this->getRequestParam('page', null);
        $viewer = Engine_Api::_()->user()->getViewer();
        if (empty($action) || empty($comment_id))
            return array();
        ...
        if (!empty($param['page']) && !empty($param['limit'])) {
            $comments->setCurrentPageNumber($param['page']);
            $comments->setItemCountPerPage($param['limit']);
        } else {
            $comments->setCurrentPageNumber(1);
            $comments->setItemCountPerPage(4);
        }
        $totoalItemCount = $comments->getTotalItemCount();
        ...
        $response = array(
            ...
            "totalReplyCount" => $totoalItemCount
        );
        return $response;
    }
