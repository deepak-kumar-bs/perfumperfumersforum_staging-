...
class Advancedactivity_IndexController extends Siteapi_Controller_Action_Standard {
    ...
    public function updateSaveFeedAction() {
        ...
        if (null === ($prev = $table->getSaveFeed($viewer, $this->_action_id)) || false === $prev)
            $isFeedSaved = 1;
    ...
    function deleteAction() {
        ...
        if (!$comment_id && (
                $activity_moderate || $is_owner || ('user' == $this->_action->subject_type && $viewer->getIdentity() == $this->_action->subject_id) || // owner of profile being commented on
            ...
            $db->beginTransaction();
            if ($activity_moderate || $is_owner || ('user' == $comment->poster_type && $viewer->getIdentity() == $comment->poster_id) || ('user' == $this->_action->object_type && $viewer->getIdentity() == $this->_action->object_id)) {
                try {
    ...
    public function likeAction() {
        ...
        try {
            ...
            if (!$comment_id) {
                ...
                $canComment = ($this->_action->getTypeInfo()->commentable && $this->_action->commentable && $viewer->getIdentity() && Engine_Api::_()->authorization()->isAllowed($this->_action->getCommentObject(), null, 'comment'));
...
//    $this->respondWithSuccess($bodyArray);
    ...
    public function sendLikeNotitficationAction() {
        ...
        try {
            ...
            //reaction code end
            // Add notification for owner of activity (if user and not viewer)
            $action_id = $this->_action->getIdentity();
            $userIds = array();
            ...
            if ($this->_action->subject_type == 'user' && $this->_action->subject_id != $viewer->getIdentity() && !in_array($this->_action->subject_id, $userIds)) {
                $actionOwner = Engine_Api::_()->getItemByGuid($this->_action->subject_type . "_" . $this->_action->subject_id);
...
//                $this->respondWithError('unauthorized');
    ...
    public function commentAction() {
        ...
        try {
            ...
            // Check authorization
            if (!Engine_Api::_()->authorization()->isAllowed($this->_action->getCommentObject(), null, 'comment'))
                $this->respondWithError('unauthorized');
...
//            // @todo we should probably limit this
	   ...
	   $nestedcomment =  Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('nestedcomment');
            $canComment = Engine_Api::_()->authorization()->isAllowed($this->_action->getCommentObject(), null, 'comment');
            $canDelete = Engine_Api::_()->authorization()->isAllowed($this->_action->getCommentObject(), null, 'edit');
            ...
            $commentInfo['gutterMenu'] = $this->_gutterMenus($this->_action,$canDelete,$poster,$comment);
    ...
    public function addCommentNotificationsAction() {
        ...
        try {
            ...
            // Add notification for owner of activity (if user and not viewer)
            if ($this->_action->subject_type == 'user' && $this->_action->subject_id != $viewer->getIdentity() && !in_array($this->_action->subject_id, $userIds)) {
                $notifyApi->addNotification($actionOwner, $viewer, $this->_action, $comment_type, array(
                    'label' => 'post'
                ));
                 $userIds [] = $this->_action->subject_id;
            }
//             Add a notification for all users that commented or like except the viewer and poster
//             @todo we should probably limit this
             $comment_type = 'commented_commented';
            if($comment->parent_comment_id){
                $comment_type = 'replied_replied';
            }
            
            $commentedUserNotifications = array();
            foreach ($this->_action->comments()->getAllCommentsUsers() as $notifyUser) {
                if ($notifyUser->getIdentity() != $viewer->getIdentity() && $notifyUser->getIdentity() != $actionOwner->getIdentity() && !in_array($notifyUser->getIdentity(), $userIds)) {
                    $commentedUserNotifications[] = $notifyUser->getIdentity();
                    $notifyApi->addNotification($notifyUser, $viewer, $this->_action, $comment_type, array(
                        'label' => 'post'
                    ));
                }
            }
...
//            // @todo we should probably limit this
            ...
            $comment_type = 'liked_commented';
            if($comment->parent_comment_id){
                $comment_type = 'liked_replied';
            }
            ...
            if (isset($composerDatas['tag']) && !empty($composerDatas['tag']))
                parse_str($composerDatas['tag'], $tagsArray);
            if (!empty($tagsArray)) {
...
// Update row
    ...
    private function _gutterMenus($action,$canDelete,$poster,$comment){
        $viewer = Engine_Api::_()->user()->getViewer();
        ...
        $canCopy = Engine_Api::_()->getApi('Siteapi_Core', 'advancedactivity')->canCopy($comment);
        if($canCopy)
        $menus[] = array(
                "name" => "comment_copy",
