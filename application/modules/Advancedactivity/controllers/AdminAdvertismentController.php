...
class Advancedactivity_AdminAdvertismentController extends Core_Controller_Action_Admin {
    ...
    public function indexAction() {
        ...
        if (array_key_exists('core_adv_types', $values)) {
         ...
         if (isset($values['advancedactivity_sitead_adv'])) {
            Engine_Api::_()->getApi('settings', 'core')->setSetting('advancedactivity.sitead.adv', $values['advancedactivity_sitead_adv']);
        }

        if (isset($values['advancedactivity_sitead_adselect'])) {
            Engine_Api::_()->getApi('settings', 'core')->setSetting('advancedactivity.sitead.adselect', $values['advancedactivity_sitead_adselect']);
        }

         if (array_key_exists('site_adv_types', $values)) {
            Engine_Api::_()->getApi('settings', 'core')->setSetting('advancedactivity.siteads', is_array($values['site_adv_types']) ? json_encode($values['site_adv_types']) : 0);
        }
