...
<?php if( $this->enableComposer ): ?>
    ...
    <div class="adv_post_container nolinks adv_post_container_<?php echo $this->statusBoxDesign ?>" id="activity-post-container" style="display: none;" >
      <form method="post" action="<?php echo $this->url(array('module' => 'advancedactivity', 'controller' => 'index', 'action' => 'post'), 'default', true) ?>" enctype="application/x-www-form-urlencoded" id="activity-form">
        ...
        <?php 
            ...
            $checkVersion = Engine_Api::_()->seaocore()->checkVersion($coreVersion, '5.3.0');
            if($checkVersion){
              $maxlength = Engine_Api::_()->getApi('settings', 'core')->getSetting('activity_postLength',1000);
              if(!empty($maxlength)){
                $maxLengthAtr = "maxLength = '".$maxlength."'";
              }
            }
        ?>
        ...
        <div class="adv_post_container_box seaocore_box_sizing">
          <div class="adv_post_box">
            ...
            <textarea id="advanced_activity_body" <?php echo $maxLengthAtr ?> cols="1" rows="1" name="body" placeholder="<?php echo $this->escape($this->translate('Post Something...')) ?>" ></textarea>
            <p id="advanced_activity_body_error_msg"></p>
          </div>
  ...
  <?php if($this->categoriesList && count($this->categoriesList) ): ?>
...
function showHideError() {
  ...
  if($('advanced_activity_body_error_msg')){
    if(maxLength == $('advanced_activity_body').value.length){
      $('advanced_activity_body_error_msg').innerHTML = "<?php echo $this->translate("You have reached maximum characters."); ?>";
    } else{
