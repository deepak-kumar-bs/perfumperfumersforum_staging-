...
<?php if(!empty($viewer_id)): ?>
  ...
  <?php if( !$this->feedOnly && !$this->onlyactivity ): ?>
        aafMoreEditOptionsSwitch = function (el) {
...
$showSuggestionFriend = (!Engine_Api::_()->core()->hasSubject() && Engine_Api::_()->user()->getViewer()->getIdentity() && $settings->getSetting('advancedactivity.friend.suggestion', 1)) ?  0: 1000;
$suggestFriendModule = Engine_Api::_()->hasModuleBootstrap('peopleyoumayknow') ? 'Peopleyoumayknow' : (Engine_Api::_()->hasModuleBootstrap('suggestion') ? 'Suggestion' : false);

?>
<?php if( !$this->feedOnly && !$this->onlyactivity ): ?>
<ul class='feed <?php echo $this->settings("aaf.showFullBoxImage", "1") ? "show_full_box_image"  :"show_org_image"?> feed_sections_<?php echo $this->feedSettings['memberPhotoStyle'] ?> <?php echo !empty($this->feedSettings['pinboardColumn']) ? '_feed_sections_pinboard feed_sections_pinboard_col_'.$this->feedSettings['pinboardColumn'].' ' : ''?>' id="activity-feed">
  <?php endif ?>
  <?php
      ...
      <?php if( !$this->noList ): ?>
        ...
        <div class="aaf_feed_top_section aaf_feed_section_<?php echo $this->feedSettings['memberPhotoStyle'] ?>">
          ...
          <div class="aaf_feed_top_section_title">
            <?php 
            if(!empty($viewer_id) && $this->onlyPreview != 1) : 
...
<!--          <div>
        ...
        <?php
          $feedAdvCount++;
          if( $feedAdvCount == $this->advCount && $displayAdd):
            $feedAdvCount = 0;
            $contentOfAdd = $this->content()->renderWidget("advancedactivity.community-ads", array('integrateCommunityAdv' => $displayAdd, 'integrateBoostFeed' => false));?>
            <?php if(strlen($contentOfAdd) > 20): ?>
              <li class="activity-item">
                <?php echo $contentOfAdd; ?>
              </li>
            <?php endif; ?>
               <?php $contentOfAdd =  $this->content()->renderWidget("advancedactivity.community-ads", array('integrateCommunityAdv' => $displayAdd, 'integrateBoostFeed' => true));?>
               <?php if(strlen($contentOfAdd) > 50): ?>
              <li class="activity-item">
                <?php echo $contentOfAdd; ?>
              </li>
            <?php endif; ?>
            <?php elseif($suggestFriendModule && ++$showSuggestionFriend == 2 && !$this->feedOnly && !$this->onlyactivity): ?>
              <?php $contentOfSuggestionFriend =$this->content()->renderWidget($suggestFriendModule.".suggestion-friend", array('getLayout' => 1, 'getWidAjaxEnabled' => 0, 'getWidLimit' => 6)); ?>
              <?php if(strlen($contentOfSuggestionFriend) > 20): ?>
              <li class="people_you_may_know_aaf_feed activity-item">
                <div class="people_you_may_know_aaf_feed_wrapper">
                <h3><?php echo $this->translate('People You May Know') ?></h3>
                <div class="scrollbars" id="people_you_may_know_aaf_feed_wrapper" style="box-sizing: border-box">
                <?php echo $contentOfSuggestionFriend; ?>
                </div>
                </div>
              </li>
              <script type="text/javascript">
                en4.core.runonce.add(function() {
                  var $el = $('people_you_may_know_aaf_feed_wrapper');
									$el.getElement('.suggestion_view_all').getParent('div').setStyle('overflowX', '');
                  $el.getElement('.suggestion_view_all').inject($el, 'before');
                  $el.scrollbars && $el.scrollbars({
                    scrollBarSize: 6,
                    fade: true,
                    barOverContent: true
                  });
                  var scrollBar = $el.retrieve('scrollbars');                 
