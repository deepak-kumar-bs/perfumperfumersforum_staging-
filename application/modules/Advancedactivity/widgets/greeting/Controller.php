  ...
  public function todaysBirthday($viewer) {
      ...
      if(!empty($membershipIdsIncViewer) && !empty($birthday_column)) {
        ...
        foreach ($birthdays as $value){
            if ($request->getCookie('has_seen_user_' . $value['item_id'])) continue;
            $birthdayUserIds[] = $value['item_id'];
        }
        // unset own birthday greeting, if closed once...
        if ($request->getCookie('has_seen_own') && ($userOwnId = array_search($viewer_id, $birthdayUserIds)) !== false ) {
          unset($birthdayUserIds[$userOwnId]);
        }
      }
