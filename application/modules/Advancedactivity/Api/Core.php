...
//      return;
  ...
  public function getComposerMenuListTitle($type, $module) {
    $title = array('photo' => 'Add Photo', 'link' => 'Add Link', 'video' => 'Add Video', 'music' => 'Add Music', 'event' => 'Create Event', 'checkin' => 'Share Location', 'hashtag' => 'Hashtag','feeling'=>'Feeling / Activity','sell'=>'Sell Something','sticker'=>'Post a Sticker','banner' => 'Add Banner','feed-tags' => 'Tag Friends','sitecrowdfunding' => 'Start a Project', 'gifPhoto' => 'Add GIF Image', 'sitelivestreaming' => 'Go Live');
    ...
    if($moduleInfo->name == 'sitelivestreaming'){
        $type = 'sitelivestreaming';
    }
    if (isset($title[$type]))
...
//                ->joinLeft($tableUser, "$rName.owner_id = $tableUser.user_id", 'username')
 ...
 //RETURNS DATE OR TIME DEPEND ON THE $DATETIME PARAMTER DATABASE TO CURRENT USER
    ...
    public function getOldVersionImages($array = array()){
        // Get available files
        $imgOptions = $array;
    ...
    public function getNewVersionImages($array){
        $fileData = Engine_Api::_()->getDbTable('files', 'core')->getFiles(array('fetchAll' => 1, 'extension' => array('gif', 'jpg', 'jpeg', 'png')));
        $imgOptions = array();
        if(!empty($array))
            $imgOptions = $array;
        else
