            ...
            if (isset($params['flag']) && !empty($params['flag'])) { // Make a feed type body params for dynamic Feed Title                               
                if (isset($helperArgs[0]) && !empty($helperArgs[0])) {
                    ...
                    if (is_object($helperArgs[0])) {
                        ...
                        $tempParams['label'] = str_ireplace( array("<br>","<br />","<br/>"), "\n", $tempParams['label'] );
    ...
    private function _activityText($data) {
        ...
        foreach ($actions as $action) {
            try { // prevents a bad feed item from destroying the entire page
                ...
                } elseif (is_numeric($network_type[0]) && count($network_type) > 0) {
                ...
                // feeling Work
                if (isset($action->params) && !empty($action->params)) {
                    if (isset($activityFeedArray['feed']['params']) && !empty($activityFeedArray['feed']['params'])) {
                        $feelingActivity = $this->formatFeelingParam($activityFeedArray['feed']['params']);
                    } else
...
//End work of publish date
                ...
                $activityFeedArray['feed']['showVerifyIcon'] = Engine_Api::_()->getApi('Core', 'siteapi')->getVerifyInfo($action->getSubject());

                // Set feed subject information on request
                // for Subject Info.................................................
                $subjectObj = $action->getSubject();
                if (!empty($subjectObj)) {
                    if ((strstr($action->getSubject()->getPhotoUrl('thumb.main'), 'http')) || (strstr($action->getSubject()->getPhotoUrl('thumb.main'), 'http'))) {
                        $getHost = '';
                    }
                    $activityFeedArray['feed']['subjectIformation']["image"] = ($action->getSubject()->getPhotoUrl('thumb.main')) ? $getHost . $action->getSubject()->getPhotoUrl('thumb.main') : '';
                    // Add cover photo in case of user cover photo.
                    if (Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('siteusercoverphoto')) {
                        $getUserCoverPhoto = Engine_Api::_()->getApi('Siteapi_Core', 'siteusercoverphoto')->getCoverPhoto($subjectObj);
                        if (!empty($getUserCoverPhoto))
                            $activityFeedArray['feed']['subjectIformation']['cover'] = $getUserCoverPhoto;
                        $activityFeedArray['feed']['subjectIformation']['displayname'] = $subjectObj->getTitle();
                        $activityFeedArray['feed']['subjectIformation']['subject_id'] = $subjectObj->getIdentity();
                    }
                }
                $getHost = $getPhotoHost;
                //..........................................................
                ...
                if (isset($data['subject_info']) && !empty($data['subject_info'])) {
                    ...
                    if (isset($activityFeedArray['feed']['subject']) && !empty($activityFeedArray['feed']['subject']))
                        $activityFeedArray['feed']['subject'] = $this->_sanitizeArray($activityFeedArray['feed']['subject']);
                ...
                if (isset($data['object_info']) && !empty($data['object_info'])) {
                    ...
                    if (isset($activityFeedArray['feed']['object']) && !empty($activityFeedArray['feed']['object']))
                        $activityFeedArray['feed']['object'] = $this->_sanitizeArray($activityFeedArray['feed']['object']);
                    ...
                    try {
                        ...
                        $activityFeedArray['feed']['object']["owner_url"] = '';
                        $activityFeedArray['feed']['object']["owner_title"] = '';
                    }
                }
                ...
                if ($action->getTypeInfo()->attachable && $action->attachment_count > 0) {
                    if (false && $action->getAttachments()) {
                        ...
                        foreach ($action->getAttachments() as $attachment) {
                            ...
                            if ($action->type == 'share' || $action->type == 'like_activity_action' || $action->type == 'comment_activity_action') {
                                try {
                                    $parentObj = $attachment->item->getParent();
                                    $tempAttachmentArray['id'] = $parentObj->getIdentity();
                                    $post_share_data = $this->_actionAsAttachment($attachment->item);
                                    if (!empty($post_share_data))
                                        $tempAttachmentArray['shared_post_data'] = $post_share_data;
                                } catch (Exception $ex) {
                            ...
                            if ($attachment->meta->mode == 0) {
                                ...
                                if ($tempAttachmentArray["attachment_type"] == 'activity_action') {
                                    ...
                                    if (isset($attachedActionObject) && !empty($attachedActionObject) && $attachedActionObject->type == 'status' && $attachedActionObject->object_type == 'user'
                                    ) {
                                ...
                                if (strstr($tempAttachmentArray["attachment_type"], 'video') && isset($attachment->item->type) && !empty($attachment->item->type)) {
                                    ...
                                    if ($tempAttachmentArray['attachment_video_type'] == 1) {
                                        $tempAttachmentArray['attachment_video_code'] = $attachment->item->code;
                                    }
                                    try {
                                        ...
                                        if (isset($contentUrl) && !empty($contentUrl)) {
                                            if(Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sitevideo')) {
                                                $contentUrl['content_url'] = $contentUrl['content_url']. '/videoType/'.Engine_Api::_()->getApi('Siteapi_Core', 'sitevideo')->videoType($attachment->item->type);
                                                $tempAttachmentArray['content_url'] = $contentUrl['content_url'];
                                            }
                                            else
                                            {
                                                $contentUrl['content_url'] = $contentUrl['content_url']. '/videoType/'.Engine_Api::_()->getApi('Siteapi_Core', 'video')->videoType($attachment->item->type);
                                                $tempAttachmentArray['content_url'] = $contentUrl['content_url'];
                                            }
                                    ...
                                    try
                                    {
                                        ...
                                        ->where('parent_id = ?', $attachment->item->getIdentity())
                                        ->limit(1);
                                        $filterImage = Engine_Api::_()->getItemTable('storage_file')->fetchRow($select);
                                        if(!empty($filterImage)){
                                        $filterImage = $this->getPhotoUrl($filterImage);
                                        $tempAttachmentArray['video_overlay_image']= $filterImage;
                                        $tempAttachmentArray['video_overlay_image']['url'] = Engine_Api::_()->getApi('Siteapi_Core', 'advancedactivity')->getHostUrl($tempAttachmentArray['video_overlay_image']['url']);
                                        }
                                ...
                                if (strpos($attachment->meta->type, '_photo') && $attachment->item->getType() != 'sitegifplayer_photo') {
                                    if ($attachedImageCount == 3 && empty($_REQUEST['action_id']) &&  (_ANDROID_VERSION && _ANDROID_VERSION >= '2.3')) {
                                        break;
                                    }
                                ...
                                if (empty($_GET['getAttachedImageDimention'])) {
                                    if ($action->attachment_count > 1 && $imageUrl = $attachment->item->getPhotoUrl('thumb.main')) {
                                        $imageUrl = Engine_Api::_()->getApi('Siteapi_Core', 'advancedactivity')->getHostUrl($imageUrl);
                                        $tempAttachmentArray["image_main"] = array(
                                            "src" => $imageUrl,
                                        );
                                        $imageMedium = $attachment->item->getPhotoUrl('thumb.medium');
                                        $imageMedium = Engine_Api::_()->getApi('Siteapi_Core', 'advancedactivity')->getHostUrl($imageMedium);
...
//                                        }
                            ...
                            if (strstr($tempAttachmentArray['uri'], 'advancedalbum')) {
                                $tempAttachmentArray['uri'] = str_replace('advancedalbums', "albums", $tempAttachmentArray['uri']);
                            }
                ...
                $canComment = ($action->getTypeInfo()->commentable && $action->commentable && $viewer->getIdentity() && Engine_Api::_()->authorization()->isAllowed($action->getCommentObject(), null, 'comment'));
                //for schedule post.............
                if (isset($_REQUEST['filter_type']) && $_REQUEST['filter_type'] == 'schedule_post') {
                    $canComment = 0;
                }
                ...
                if (_ANDROID_VERSION >= '2.3' || _IOS_VERSION >= '2.3.1') {
                    $tempActivityMenu = array();
                    ...
                    $tempActivityMenu['feed_link'] = $activityFeedArray['feed_link'];
...
//                    }
                    ...
                    if ($viewer_id && (
                            ...
                            $data['allow_delete'] && (
                            ('user' == $action->subject_type && $viewer_id == $action->subject_id) || ('user' == $action->object_type && $viewer_id == $action->object_id)
                            )
                            )
                            ) && !empty(Engine_Api::_()->getApi('settings', 'core')->getSetting('activity.userdelete'))) {
                    ...
                    if ((!empty($is_owner)  && !empty(Engine_Api::_()->getApi('settings', 'core')->getSetting('activity.userdelete'))) || (isset($viewer->level_id) && ($viewer->level_id == 1))) {
                        $tempActivityMenu = array();
                        $tempActivityMenu["name"] = "delete_feed";
                        $tempActivityMenu["label"] = Engine_Api::_()->getApi('Core', 'siteapi')->translate('Delete Feed');
                        $tempActivityMenu["url"] = "advancedactivity/delete";
               ...
               $activityFeedArray['feed']['action_type_body'] = @html_entity_decode(Engine_Api::_()->getApi('Core', 'siteapi')->translate(@trim(strip_tags($getFeedTypeInfo['body']))), ENT_QUOTES, "utf-8");
                ...
                $modifiedActionTypeBodyParams = array();
                ...
                $activityFeedArray['feed']['action_type_body'] = strip_tags($final_action_type_body);
...
//
//                            $getDefaultKey = $key;
                        } elseif ($paramArray['search'] == '{var:$type}' && $activityFeedArray['feed']['type'] == 'share' && $activityFeedArray['feed']['share_params_type'] == 'activity_action') {
                            $paramArray['type'] = $activityFeedArray['feed']['share_params_type'] . '_app_post';
                            $paramArray['id'] = $activityFeedArray['feed']['share_params_id'];
                            $finalArray[] = $paramArray;
                        } else {
                ...
                //Icon redirection .............................
                if ($activityFeedArray['feed']['action_type_body_params']) {
                    $feedTypeObj = $activityFeedArray['feed']['action_type_body_params'][0];
                    if ($feedTypeObj['type'] == 'user')
                        $activityFeedArray['feed']['feed_type'] = 0;
                    else {
                        $activityFeedArray['feed']['feed_type'] = 1;
                    }
                }
    ...
    private function getHashtagNames($action) {
        ...
        foreach ($hashtagmaps as $hashtagmap) {
            ...
            if (isset($hashtags) && is_array($hashtags) && !empty($hashtags)) {
                if ($tag && !in_array($tag->text, $hashtags)) {
...
//        }
    ...
    public function isSitereactionPluginLive() {
        if (Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sitereaction')) {
            ...
            if (!empty(Engine_Api::_()->siteapi()->checkVersion($reactionModule, '4.8.12p4'))) {
    ...
    public function videoSource() {
        ...
        if (Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sitevideo')) {
            try {
                ...
                $key = Engine_Api::_()->getApi('settings', 'core')->getSetting('sitevideo.youtube.apikey', $coreSettings->getSetting('video.youtube.apikey'));
        ...
        try {
            ...
            if (isset($param) && !empty($param)) {
                if (is_array($param) && count($param) > 0) {
                    $banner = isset($param['feed-banner']) ? $param['feed-banner'] :array();
                    if (empty($banner)) {
                        $param['feed-banner'] = '';
                        return $param;
                    }
                } else {
                    $param = $temp = json_decode($param);
                ...
                if (!empty($finalurl)) {
                    if (strstr($finalurl, "radial-gradient") || $banner['image'] == 'none')
    ...
    public function formatFeelingParam($param = array()) {
        ...
        try {
            ...
            if (isset($param) && !empty($param)) {
                if (is_array($param) && count($param) > 0) {
                    if (!isset($param['feelings']) || empty($param['feelings']))
                        return false;
                ...
                if (!empty($feelingtype)) {
                    $param['feelings']['parenttitle'] = Engine_Api::_()->getApi('Core', 'siteapi')->translate($feelingtype->getTitle());
    ...
    public function getPhotoUrl($item, $type = null) {
        ...
        if (strstr($item->getType(), 'core_link') && empty($item->photo_id)) {
            if (is_array($item->params) && isset($item->params['iframely']) && isset($item->params['iframely']['links'])) {
                if (!empty($item->params['iframely']['links']['thumbnail'])) {
                    $imageUrl = $item->params['iframely']['links']['thumbnail'][0]['href'];
                    $size = isset($item->params['iframely']['links']['thumbnail'][0]['media']) ? $item->params['iframely']['links']['thumbnail'][0]['media'] : "";
                    if (empty($size)) {
                        $size = array();
                        $size['width'] = 0;
                        $size['height'] = 0;
                    ...
                    return $array;
                } elseif (!empty($item->params['iframely']['links']['file'])) {
                    $imageUrl = $item->params['iframely']['links']['file'][0]['href'];
                    $size = array();
                    $size['width'] = 0;
                    $size['height'] = 0;
                    $array = array(
        ...
        if ($item->getType() == 'sitestoreproduct_wishlist') {
            $type = 'thumb.main';
            $item = Engine_Api::_()->getItem('sitestoreproduct_product', $item->product_id);
        }

        if ($item->getType() == 'sitereview_wishlist') {
            $type = 'thumb.main';
            $item = Engine_Api::_()->getItem('sitereview_listing', $item->listing_id);
            if(empty($item))
           return array();
        }
        ...
        if ($item->getType() == 'album' || $item->getType() == 'sitealbum') {
            ...
            $coreMusic = Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled("music");
            if (!empty($coreMusic))
                $playlist = Engine_Api::_()->getItem('music_playlist', $item->playlist_id);
            else
                $playlist = Engine_Api::_()->getItem('sitemusic_pl', $item->playlist_id);
            if (!empty($playlist))
            ...
            elseif ($item->thumbnail) {
                ...
                $array = array(
                    'url' => $url,
                    'params' => array('width'=> 220, 'height' => 150)
                );
                return $array;
            }
...
// YouTube
        ...
        if ($video->type == 1 || $video->type == 'youtube') {
        ...
        } elseif ($video->type == 3 || $video->type == 'upload' || $video->type == 'mydevice' || $video->type == 'stream') { // Uploded Videos
            $staticBaseUrl = Engine_Api::_()->getApi('settings', 'core')->getSetting('core.static.baseurl', null);
            ...
            if(!empty(Engine_Api::_()->storage()->get($video->file_id, $video->getType())))
            ...
            if (!strstr($video_location, 'http'))
                $video_location = $getParentHost . DIRECTORY_SEPARATOR . $baseParentUrl . $video_location;
    ...
    public function getPhotoTag($photo) {
        ...
        foreach ($photo->tags()->getTagMaps() as $tagmap) {
            if (($viewer->getIdentity() == $tagmap->tag_id) || ($photo->owner_id == $viewer->getIdentity()) || ($viewer->getIdentity() == $tagmap->tagger_id)) {
                $isRemove = 1;
            } else
                $isRemove = 0;
    ...
    private function _actionAsAttachment($item = null) {
        if (empty($item))
            return;
        if ($item->getType() == 'activity_action') {
            //$this->_actionAsTtachment();
            $actionTable = Engine_Api::_()->getDbtable('actions', 'advancedactivity');
            $actionAttch = $actionTable->getActionById($item->getIdentity());
            if (!empty($actionAttch)) {
                $activityAttch[] = $actionAttch;
                if (isset($_REQUEST['subject_info'])) {
                    $subject_info = $_REQUEST['subject_info'];
                } else {
                    $subject_info = false;
                }
                if (isset($_REQUEST['object_info'])) {
                    $object_info = $_REQUEST['object_info'];
                } else {
                    $object_info = false;
                }

                $feed_attached = $this->getFeeds($activityAttch, array(
                    "subject_info" => $subject_info,
                    "object_info" => $object_info,
                ));
            }

            if (!empty($feed_attached)) {
                if (isset($feed_attached[0]['feed_menus'])) {
                    unset($feed_attached[0]['feed_menus']);
                }
                if (isset($feed_attached[0]['feed_footer_menus'])) {

                    unset($feed_attached[0]['feed_footer_menus']);
                }
            }
            return $feed_attached;
        }
    }

    public function postVideo($values) {
        $viewer = Engine_Api::_()->user()->getViewer();
    ...
    private function _checkYouTube($code) {
        $key = Engine_Api::_()->getApi('settings', 'core')->getSetting('sitevideo.youtube.apikey', Engine_Api::_()->getApi('settings', 'core')->getSetting('video.youtube.apikey'));
    ...
    private function _handleInformation($type, $code) {
     ...
     private function _sanitizeArray($dataArray = array(), $ignoreParams = array()) {
        $restrictedFields = array('email', 'password', 'salt', 'creation_ip', 'lastlogin_ip');
        foreach ($restrictedFields as $restrictedValue) {
            if (!in_array($restrictedValue, $ignoreParams) && array_key_exists($restrictedValue, $dataArray)) {
