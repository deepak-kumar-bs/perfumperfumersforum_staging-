    ...
    <div class="sr_profile_photo_wrapper b_medium <?php if ($photo_type == 'listing'): ?>photo_zoom<?php endif; ?>">
      ...
      <div class='sr_profile_photo <?php if ($this->can_edit && $photo_type == 'listing'): ?>sr_photo_edit_wrapper<?php endif; ?>'>
        ...
        <?php if($this->sitereview->photo_id ):?>
            <?php $photo= $this->sitereview->getPhotoUrl(); ?>
            <?php if($photo_type == 'listing'):?>
              <a href="<?php echo $photo ?>" <?php if (SEA_LIST_LIGHTBOX) : ?> onclick='openSeaocoreLightBox("<?php echo $photo ?>");return false;' <?php endif; ?>>
      ...
      <?php if($this->sitereview->photo_id && ($photo_type == 'listing')):?>
        <p class="mtop10">
          ...
          <?php if(in_array('photo', $this->showContent) && in_array('photosCarousel', $this->showContent)&&strlen($widgetContent)>15):?>
            <?php echo $this->translate('Click on images to view full size.'); ?>
          <?php else: ?>
            <?php echo $this->translate('Click on image to view full size.'); ?>
          <?php endif; ?>
          </a>
        </p>
     <?php endif; ?>
  ...
  <div class="sr_profile_content">
    ...
    <?php if (($this->phone) || ($this->email) || ($this->website)) : ?>
          ...
          <div class="sr_profile_contect_op">
            ...
            <span id="showEmailAddress" class="o_hidden">
              <?php if ( empty($this->emailme) ) :?> 
                <a href='mailto:<?php echo $this->email ?>' title="<?php echo $this->email ?>"><?php echo $this->translate('Email Me') ?></a>
              <?php else: ?> 
                <a href="javascript:void(0);" onclick="Smoothbox.open('<?php echo $this->escape($this->url(array('route' => 'default', 'module' => 'sitereview', 'controller' => 'profile', 'action' => 'email-me', "id" => $this->sitereview->listing_id), 'default' , true)); ?>'); return false;"><?php echo $this->translate('Email Me') ?></a>
              <?php endif; ?> 
            </span>      
		      </div>
		    <?php endif; ?>
