...
class Sitereview_Installer extends Engine_Package_Installer_Module {
    ...
    function onInstall() {
        ...
        $sitereviewVersion = $this->getSitereviewVersion();
        if($sitereviewVersion->version <= '4.10.3p23' )  {
            $db->query("ALTER TABLE `engine4_sitereview_categories` ADD `photo_id` INT NOT NULL DEFAULT '0' AFTER `subcat_dependency`;");
        }
        $select = new Zend_Db_Select($db);
...
('SITEREVIEW_BECOMEOWNER_EMAIL', 'sitereview', '[host],[email],[list_title],[object_link],[listing_type],[list_title_with_link], [site_contact_us_link]')");
        ...
        if ($tableListings) {
            ...
            $sitereview_parent_type = $db->query("SHOW COLUMNS FROM engine4_sitereview_listings LIKE 'parent_type'")->fetch();
            if (empty($sitereview_parent_type)) {
                $db->query("ALTER TABLE `engine4_sitereview_listings` ADD `parent_type` VARCHAR(128) NULL DEFAULT NULL AFTER `body`, ADD `parent_id` INT NULL DEFAULT NULL AFTER `parent_type`; ");
            }
        }
...
('sitereview','1')");
    ...
    private function getSitereviewVersion()   {
        $db = $this->getDb(); 
        $select = new Zend_Db_Select($db);
        $select->from('engine4_core_modules', array('title', 'version'))
               ->where('name = ?', 'sitereview')
